...
export const SendNearToken = () => {
    ...
    const amoutInputHandler = async (e: any) => {
        ...
        if (parseFloat(amount) > nearBalance) {
            setAmoutError(true)
            return;
        }
    ...
    useEffect( () => {
        const getBalance = async() => {
            const api = await getAccountNearBalance({
                accountId: accountId,      
            })

            const response = await api.json()
            setNearBalance(response.data.data.balance)
        }
    ...
    return(<>
        <section>
            <div id='importAccount' className=''>
                ...
                <div id='submit-btn'>
                    <button type='button' onClick={sendNearHandler}>send</button>
                </div>
            </div>
        </section>
    </>)
